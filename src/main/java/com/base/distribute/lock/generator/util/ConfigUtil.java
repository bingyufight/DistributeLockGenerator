...
public class ConfigUtil {
	 ...
	 * zookeeper目标配置文件名
	 */
	private static String zookeeperConfigFileName = "zk_config.properties";
	
	
	/**
	 * redis目标配置文件名
	 */
	private static String redisConfigFileName = "redis_config.properties";
	 ...
	 * 目标配置文件名
	 */
	private static String configFileName = zookeeperConfigFileName;
	...
	public static Properties getProperties(String source) {
		if (proPerties.get(source) == null) {
			synchronized (ConfigUtil.class) {
				if (proPerties.get(source) == null) {
					if(DistributeLockType.ZOOKEEPER.getValue().equals(source)){
						configFileName = zookeeperConfigFileName;
					} else if (DistributeLockType.REDIS.equals(source)){
						configFileName = redisConfigFileName;
					}
					Properties PROPERTIES = ConfigUtil.loadProperties(configFileName, false, false);
					proPerties.put(source, PROPERTIES);
	...
	public static String getProperty(String source, String key) {
		return getProperties(source).getProperty(key, null);
	}

	public static String getProperty(String source,String key, String defaultValue) {
		return getProperties(source).getProperty(key, defaultValue);
	}
